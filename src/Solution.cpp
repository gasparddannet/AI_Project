...
using namespace std;
Solution::Solution(vector<vector<int>> solution) : solution(solution) {}
...
Solution &Solution::operator=(const Solution &s)
{
    if (this != &s) // Avoid self-assignment
    {
        ...
        {
            this->solution[i].resize(s.solution[i].size());
            for (int j=0; j<s.solution[i].size(); j++)
            {
                this->solution[i][j] = s.solution[i][j];
            }
        }
    }
    return *this;
...
vector<vector<int>> Solution::getSolution()
{
    return solution;
}
